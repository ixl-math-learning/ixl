importScripts('/ultraviolet/uv/uv.bundle.js');
importScripts('/ultraviolet/uv/uv.config.js');
...
class RequestContext {
    constructor(request, worker, ultraviolet, body = null) {
        ...
        this.address = worker.addresses[Math.floor(Math.random() * worker.addresses.length)];
        this.body = body || null;
    ...
    get send() {
        const targetDomain = this.address.hostname;
        const targetUrl = new URL(this.url.pathname + this.url.search, this.address);

        return new Request(targetUrl.href, {
            method: this.method,
            headers: {
                'x-bare-protocol': this.url.protocol,
                'x-bare-host': targetDomain,
