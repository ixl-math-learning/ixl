...
function sendExitIntentToUnity() {
    if (typeof unityInstance !== 'undefined') {
        console.log("[exit-intent] Sending exit intent to Unity");
        unityInstance.SendMessage('WebUtils', 'OnExitIntent');
    } else {
        console.warn("[exit-intent] Unity instance not available");
    }
}
...
function isMouseInCorner(x, y) {
    ...
    if (x <= CORNER_THRESHOLD && y <= CORNER_THRESHOLD) {
        console.log("[exit-intent] Mouse in top-left corner");
        return true;
    }
    
    // Check top-right corner
    if (x >= windowWidth - CORNER_THRESHOLD && y <= CORNER_THRESHOLD) {
        console.log("[exit-intent] Mouse in top-right corner");
        return true;
    }
