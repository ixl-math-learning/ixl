...
async function __uvHook(window, config = {}, bare = '/~r/8/') {
    ...
    client.element.on('setAttribute', event => {
        ...
        if (__uv.attrs.isSrcset(event.data.name)) {
            event.target.call(event.that, __uv.attributePrefix + '-attr-' + event.data.name, event.data.value);
            if (typeof event.data.value === 'string') event.data.value = __uv.html.wrapSrcset(event.data.value);
        };
    ...
    client.element.hookProperty([HTMLImageElement], 'srcset', {
        ...
        set: (target, that, [val]) => {
            ...
            target.call(that, typeof val === 'string' ? __uv.html.wrapSrcset(val) : val);
        },
    });
